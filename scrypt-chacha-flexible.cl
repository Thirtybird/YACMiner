/*
	...
	Conversion to flexible lookup-gap and buffer size by thirtybird
...
static void
//ySize is equivilent to ipt in the opencl code, so buffer_size to thread_concurrency is 
//thus CONCURRENT_THREADS can be calculated by (BUFFER_SIZE * 1024 * 1024) / (ySIZE * 128)
//problem ; N does not equal 1024, so ySIZE is _NOT_ equivilent to ipt
scrypt_ROMix(uint4 *restrict X/*[chunkWords]*/, __global uint4 *restrict lookup/*[N * chunkWords]*/, const uint N, const uint gid, const uint Nfactor, const uint lookupgap) {
	const uint zSIZE = 8;
...
const uint4 midstate0, const uint4 midstate16, const uint target, const uint N, const uint lookupgap)
{
	uint4 password[5];
	uint4 X[8];
	...
	/* 2: X = ROMix(X) */
	scrypt_ROMix(X, (__global uint4 *)padcache, N, gid, Nfactor, lookupgap);
